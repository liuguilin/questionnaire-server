/**
 * SW Tracking 问卷后端服务（历史/本地校验脚本，正式入口见 app.js + routes/*）
 *
 ...
 *
 * 免问卷仅定位也可走正式服务 POST /api/submit（不传 answers）；本脚本仅覆盖问卷路径。
 *
...
// 本地开发连接
const localConnectionString = 'mongodb://hkuapp:yuanziHKU240327@8.210.252.35:27017/SW_Tracking?authSource=admin';
 ...
 *
 * 注意：本文件为历史/校验脚本，正式路由见 routes/questionnaire.js。
 * 免问卷仅定位：正式服务同一 POST /api/submit 不传 answers 即可；本脚本仍按问卷路径处理。
 *
 * 请求体字段：
 *   type      - 'day' | 'night'（必填）
 *   uid       - 设备/用户唯一标识（必填）
 *   answers   - 答案数组 [{ qid, answer }]（可选；正式服务免问卷可不传）
 *   locations - 位置数组（可选；空数组/不传时更新不覆盖已有位置）
 *   timestamp - Unix 秒级时间戳（必填）
 *   enc       - 时间戳加密值（必填）
